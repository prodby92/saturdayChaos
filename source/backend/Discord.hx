...
class DiscordClient
{
	...
	private inline static final _defaultID:String = "1469751072040943798";
	...
	public static function changePresence(details:String = 'i like dih in my buh', ?state:String, ?smallImageKey:String, ?hasStartTimestamp:Bool, ?endTimestamp:Float, largeImageKey:String = 'purge_1')
	{
		var startTimestamp:Float = 0;
		if (hasStartTimestamp) startTimestamp = Date.now().getTime();
		if (endTimestamp > 0) endTimestamp = startTimestamp + endTimestamp;

		var resolvedSmallImageKey = (smallImageKey == null || smallImageKey.length == 0) ? "purge_1" : smallImageKey;
		presence.state = state;
		presence.details = details;
		presence.smallImageKey = resolvedSmallImageKey;
		presence.largeImageKey = largeImageKey;
		presence.largeImageText = "fuck outta here bro";
