...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var playerKeyCount:Int = (PlayState.SONG != null) ? Std.int(Math.max(4, Song.getKeyCountForPlayer(PlayState.SONG))) : 4;
		if (playerKeyCount == 6) {
			keysArray = [
				'note6_left',
				'note6_up',
		...
		dadGroup.add(dad);
		if (PlayState.SONG.noOpponent == true)
	...
	private function getSongColumnCountForPlayer(?player:Int = 1):Int
	{
		if (SONG == null) return 4;
		return player < 1 ? Song.getKeyCountForOpponent(SONG) : Song.getKeyCountForPlayer(SONG);
	}

	private function getAnimationSetForNote(note:Null<Note>):Array<String>
	{
		var sideCount:Int = 4;
		if (SONG != null)
		{
			sideCount = (note != null && note.mustPress) ? Song.getKeyCountForPlayer(SONG) : (note != null ? Song.getKeyCountForOpponent(SONG) : Song.getKeyCountForPlayer(SONG));
			sideCount = Std.int(Math.max(4, sideCount));
		}
		if (sideCount == 6)
			return ['singBACK', 'singUP', 'singRIGHT', 'singLEFT', 'singDOWN', 'singFORWARD'];
		return ['singLEFT', 'singDOWN', 'singUP', 'singRIGHT'];
	}

	private function getAnimationNameForNote(note:Null<Note>, lane:Int):String
	{
		var anims:Array<String> = getAnimationSetForNote(note);
		var laneIndex:Int = Std.int(Math.abs(Math.min(anims.length - 1, lane)));
		var animName:String = anims[laneIndex] + (note != null ? note.animSuffix : '');
		if (note != null && note.mustPress)
		{
			var char:Character = boyfriend;
			if (char != null && !char.hasAnimation(animName))
			{
				if (animName == 'singBACK' + note.animSuffix)
					animName = 'singLEFT' + note.animSuffix;
				else if (animName == 'singFORWARD' + note.animSuffix)
					animName = 'singRIGHT' + note.animSuffix;
			}
		}
		else if (note != null)
		{
	...
	private function generateSong():Void
	{
		...
		var opponentCount:Int = Song.getKeyCountForOpponent(songData);
		var playerCount:Int = Song.getKeyCountForPlayer(songData);
		totalColumns = Std.int(Math.max(4, opponentCount + playerCount));

		// Ensure sing animation mapping matches the active 6-key side when either side uses 6 lanes.
		if (Song.is6KeyChart(songData)) {
			// Expected six-key animation order: Back, Up, Right, Left, Down, Forward
			singAnimations = ['singBACK', 'singUP', 'singRIGHT', 'singLEFT', 'singDOWN', 'singFORWARD'];
		} else {
		...
		for (section in sectionsData)
		{
			...
			for (i in 0...section.sectionNotes.length)
			{
				...
				var playerCount:Int = Std.int(Math.max(4, Song.getKeyCountForPlayer(songData)));
				var noteColumn: Int = Std.int(rawNoteData < playerCount ? rawNoteData : rawNoteData - playerCount);
				var holdLength: Float = songNotes[2];
				var noteType: String = !Std.isOfType(songNotes[3], String) ? Note.defaultNoteTypes[songNotes[3]] : songNotes[3];
				if (Math.isNaN(holdLength))
					holdLength = 0.0;

				var gottaHitNote:Bool = (rawNoteData < playerCount);
				...
				if(roundSus > 0)
				{
					for (susNote in 0...roundSus)
					{
						...
						var sustainNote:Note = new Note(spawnTime + (curStepCrochet * susNote), noteColumn, oldNote, true, false, null, gottaHitNote);
						...
						{
							var columnsPerPlayer:Int = getSongColumnCountForPlayer(0);
							if (columnsPerPlayer < 4) columnsPerPlayer = 4;
							var visualLaneIndex:Int = Std.int(Math.abs(noteColumn) % columnsPerPlayer);
							var halfCount:Int = Std.int(Math.ceil(columnsPerPlayer / 2));
							var laneIndex:Int = Std.int(Math.abs(visualLaneIndex) % halfCount);
							var sideOffset:Float = (columnsPerPlayer * 112) + 24;
							if (visualLaneIndex < halfCount)
								sustainNote.x = (FlxG.width / 2) - sideOffset + (laneIndex * 112);
							else
								sustainNote.x = (FlxG.width / 2) + sideOffset + ((visualLaneIndex - halfCount) * 112);
						}
					}
				}
				...
				else if(ClientPrefs.data.middleScroll)
				{
					...
					var sideOffset:Float = (columnsPerPlayer * 112) + 24;
					if (visualLaneIndex < halfCount)
						swagNote.x = (FlxG.width / 2) - sideOffset + (laneIndex * 112);
					else
						swagNote.x = (FlxG.width / 2) + sideOffset + ((visualLaneIndex - halfCount) * 112);
				}
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		var keyCount:Int = getSongColumnCountForPlayer(player);
		for (i in 0...keyCount)
		{
			// FlxG.log.add(i);
			...
			if(player < 1 && PlayState.SONG != null && PlayState.SONG.noOpponent == true)
			{
	...
	public function moveCameraSection(?sec:Null<Int>):Void {
		...
		var noOpponent:Bool = (SONG != null && SONG.noOpponent == true);
		var isDad:Bool = !noOpponent && (SONG.notes[sec].mustHitSection != true);
		moveCamera(isDad);
		if (isDad)
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if(char != null && (note == null || !note.noMissAnimation) && char.hasMissAnimations)
		{
			...
			var animToPlay:String = getAnimationNameForNote(note, direction) + 'miss' + postfix;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		{
			var char:Character = dad;
			var animToPlay:String = getAnimationNameForNote(note, note.noteData);
