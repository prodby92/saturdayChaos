...
class TitleState extends MusicBeatState
{
	...
	var logoSpinSpeed:Float = 90;
	var logoIntroSlideDuration:Float = 2;
	var logoExitSlideDuration:Float = 1;
	var logoIntroTweened:Bool = false;
	var logoBaseScale:Float = 0.8;
	var logoPressScale:Float = 0.7;
	var logoPressBounceDuration:Float = 0.5;
	...
	function startIntro()
	{
		...
		logoBl = new FlxSprite().loadGraphic(Paths.image('disk'));
		logoBl.antialiasing = ClientPrefs.data.antialiasing;
		logoBl.scale.set(logoBaseScale, logoBaseScale);
		logoBl.updateHitbox();
		logoBl.screenCenter();
		logoBaseY = logoBl.y - 30;
		logoBl.y = FlxG.height + logoBl.height;
	...
	override function update(elapsed:Float)
	{
		...
		if (logoBl != null)
		{
			logoBl.angle = (logoBl.angle + (logoSpinSpeed * elapsed)) % 360;
		}
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				if(logoBl != null)
				{
					...
					logoBl.scale.set(logoPressScale, logoPressScale);
					logoBl.updateHitbox();
					logoBl.screenCenter(X);
					FlxTween.tween(logoBl.scale, {x: logoBaseScale, y: logoBaseScale}, logoPressBounceDuration, {
						ease: FlxEase.quadOut,
						onUpdate: function(twn:FlxTween) {
							logoBl.screenCenter(X);
						}
					});
					FlxTween.tween(logoBl, {y: -logoBl.height - 40}, logoExitSlideDuration, {ease: FlxEase.expoIn});
				}
	...
	function tweenLogoIntoCenter():Void
	{
		if (logoBl == null || logoIntroTweened) return;
		logoIntroTweened = true;
		FlxTween.cancelTweensOf(logoBl);
		FlxTween.tween(logoBl, {y: logoBaseY}, logoIntroSlideDuration, {ease: FlxEase.expoOut});
	}
