...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	public static function getChartingLaneCount(?song:SwagSong):Int
	{
		var source:SwagSong = song != null ? song : (PlayState.SONG != null ? PlayState.SONG : null);
		if(source != null)
		{
			var opponentCount:Int = Song.getKeyCountForOpponent(source);
			var playerCount:Int = Song.getKeyCountForPlayer(source);
			return Std.int(Math.max(4, opponentCount + playerCount));
		}
		return GRID_COLUMNS_PER_PLAYER;
	}

	public static function getChartingColumnsForSide(side:Int, ?song:SwagSong):Int
	{
		var source:SwagSong = song != null ? song : (PlayState.SONG != null ? PlayState.SONG : null);
		if(source != null)
		{
			...
			{
				var opponentCount:Int = Song.getKeyCountForOpponent(source);
				if(opponentCount > 0) return Std.int(Math.max(4, opponentCount));
			}
			else
			{
				var playerCount:Int = Song.getKeyCountForPlayer(source);
				if(playerCount > 0) return Std.int(Math.max(4, playerCount));
			}
		}
		return GRID_COLUMNS_PER_PLAYER;
	}

	public static function getChartingColumnsPerPlayer(?song:SwagSong):Int
	{
		return getChartingColumnsForSide(1, song);
	}

	public static function getChartingPlayers(?song:SwagSong):Int
	{
		var noOpponent:Bool = false;
		if (PlayState.SONG != null && PlayState.SONG.noOpponent == true) noOpponent = true;
		if (song != null && song.noOpponent == true) noOpponent = true;
		if (noOpponent) return 1;
		return GRID_PLAYERS;
	}

	public static function getChartingTotalColumns(?song:SwagSong):Int
	{
		return getChartingColumnsForSide(0, song) + getChartingColumnsForSide(1, song);
	}

	public static function getChartingVisibleTotalColumns(?song:SwagSong):Int
	{
		var noOpponent:Bool = false;
		if (PlayState.SONG != null && PlayState.SONG.noOpponent == true) noOpponent = true;
		if (song != null && song.noOpponent == true) noOpponent = true;
		if (noOpponent)
			return getChartingColumnsPerPlayer(song);
		return getChartingTotalColumns(song);
	}

	public static function getChartingVisibleColumnOffset(?song:SwagSong):Int
	{
		return 0;
	}

	public static function getChartingVisualColumnIndex(data:Int, ?song:SwagSong):Int
	{
		var playerCount:Int = getChartingColumnsForSide(1, song);
		var noOpponent:Bool = false;
		if (song != null && song.noOpponent == true) noOpponent = true;
		else if (PlayState.SONG != null && PlayState.SONG.noOpponent == true) noOpponent = true;
		if (noOpponent)
			return Std.int(Math.max(0, data));
		if (data < playerCount)
			return data;
		return playerCount + (data - playerCount);
	}

	public static function getChartingRawDataForVisualIndex(data:Int, ?song:SwagSong):Int
	{
		var playerCount:Int = getChartingColumnsForSide(1, song);
		var noOpponent:Bool = false;
		if (song != null && song.noOpponent == true) noOpponent = true;
	...
	public static function remapLaneIndex(data:Int, fromLaneCount:Int, toLaneCount:Int):Int
	{
		var oldCount:Int = Std.int(Math.max(1, fromLaneCount));
		var newCount:Int = Std.int(Math.max(1, toLaneCount));

		if(oldCount == newCount || fromLaneCount == toLaneCount) return data;

		var clamped:Int = Std.int(FlxMath.bound(data, 0, oldCount - 1));
		var ratio:Float = (clamped / Math.max(1, oldCount - 1)) * Math.max(0, newCount - 1);
		return Std.int(FlxMath.bound(Math.round(ratio), 0, newCount - 1));
	}

	public static function remapChartNoteData(data:Int, fromLaneCount:Int, toLaneCount:Int):Int
	{
		return remapLaneIndex(data, fromLaneCount, toLaneCount);
	}

	public static function remapChartNoteDataForSides(data:Int, oldPlayerCount:Int, oldOpponentCount:Int, newPlayerCount:Int, newOpponentCount:Int):Int
	{
		var oldPlayerCountSafe:Int = Std.int(Math.max(4, oldPlayerCount));
		var oldOpponentCountSafe:Int = Std.int(Math.max(4, oldOpponentCount));
		var newPlayerCountSafe:Int = Std.int(Math.max(4, newPlayerCount));
		var newOpponentCountSafe:Int = Std.int(Math.max(4, newOpponentCount));

		var isPlayerSide:Bool = data < oldPlayerCountSafe;
		var localLane:Int = isPlayerSide ? data : data - oldPlayerCountSafe;
		var oldSideCount:Int = isPlayerSide ? oldPlayerCountSafe : oldOpponentCountSafe;
		var newSideCount:Int = isPlayerSide ? newPlayerCountSafe : newOpponentCountSafe;
		var remappedLane:Int = remapLaneIndex(localLane, oldSideCount, newSideCount);

		if(isPlayerSide) return remappedLane;
		return newPlayerCountSafe + remappedLane;
	}


	public static function getChartingNoteData(data:Int, ?song:SwagSong):Int
	{
		var playerCount:Int = getChartingColumnsPerPlayer(song);
		return Std.int(data >= playerCount ? data - playerCount : data);
	}

	public static function getChartingMustPress(data:Int, ?song:SwagSong):Bool
	{
		return data < getChartingColumnsPerPlayer(song);
	}
	...
	override function create()
	{
		...
		var totalColumns:Int = getChartingVisibleTotalColumns();
		var opponentCount:Int = getChartingColumnsForSide(0);
		var playerCount:Int = getChartingColumnsForSide(1);
		for (i in 0...totalColumns)
		{
			var isPlayerSide:Bool = i < playerCount;
			var laneData:Int = isPlayerSide ? i : i - playerCount;
			var laneOwner:Int = isPlayerSide ? 1 : 0;
			var note:StrumNote = new StrumNote(startX + (GRID_SIZE * i), startY, laneData, laneOwner);
		...
		var chartPlayers:Int = getChartingPlayers();
		// build grid stripes based on the active chart player count and side widths
		for (i in 0...chartPlayers)
		{
			if(columns > 0) gridStripes.push(columns);
			var sideColumns:Int = (i == 0) ? getChartingColumnsForSide(1) : getChartingColumnsForSide(0);
			columns += sideColumns;
		}

		// Always create the full set of header icons (2 by default) so UI
		// elements remain consistent when toggling noOpponent during gameplay.
		for (i in 0...GRID_PLAYERS)
		{
			...
			var sideColumns:Int = (i == 0) ? getChartingColumnsForSide(1) : getChartingColumnsForSide(0);
			icon.x = iconX + GRID_SIZE * (sideColumns / 2) - icon.width / 2;
			iconX += GRID_SIZE * sideColumns;
		}
	...
	function refreshSongKeyCheckboxes()
	{
		if (opponentSixKeyCheckBox != null)
			opponentSixKeyCheckBox.checked = (PlayState.SONG != null && Song.getKeyCountForOpponent(PlayState.SONG) == 6);
		if (playerSixKeyCheckBox != null)
			playerSixKeyCheckBox.checked = (PlayState.SONG != null && Song.getKeyCountForPlayer(PlayState.SONG) == 6);
	}
	...
	function onChartLoaded()
	{
		...
		refreshSongKeyCheckboxes();
		noOpponentCheckBox.checked = (PlayState.SONG.noOpponent == true);
	...
	inline function hasActiveTextInput():Bool
	{
	...
	override function update(elapsed:Float)
	{
		...
		ClientPrefs.toggleVolumeKeys(!hasActiveTextInput());
		...
		{
			if(!hasActiveTextInput()) //If not typing anything
			{
		...
		if(!hasActiveTextInput() && lastFocus == null)
		{
			...
			if(!hasActiveTextInput() && (doCut || FlxG.keys.justPressed.DELETE || FlxG.keys.justPressed.BACKSPACE || (isMovingNotes && (FlxG.mouse.justPressedRight || FlxG.keys.justPressed.ESCAPE)))) // Delete button
			{
				if(selectedNotes.length > 0)
				{
		...
		if(FlxG.mouse.x >= minX && FlxG.mouse.x < gridBg.x + gridBg.width)
		{
			...
			if(isMovingNotes)
			{
				...
				if(movingNotesLastData != nData)
				{
					...
					var maxn:Int = getChartingTotalColumns() - 1;
			...
			else if(FlxG.mouse.justPressed && !ignoreClickForThisFrame)
			{
				...
				else if(FlxG.mouse.x >= gridBg.x && FlxG.mouse.x < gridBg.x + gridBg.width)
				{
					var closeNotes:Array<MetaNote> = curRenderedNotes.members.filter(function(note:MetaNote)
					{
						...
						var visibleIndex:Int = getChartingVisualColumnIndex(Std.int(note.songData[1]));
						return ((note.isEvent && noteData < 0) || (!note.isEvent && visibleIndex == noteData)) && chartY >= 0 && chartY < GRID_SIZE;
					});
					...
					else if(!holdingAlt && FlxG.mouse.y >= gridBg.y && FlxG.mouse.y < gridBg.y + gridBg.height) // Add note
					{
						...
						var visibleOffset:Int = getChartingVisibleColumnOffset();
						var noteSetupData:Array<Dynamic> = [strumTime, getChartingRawDataForVisualIndex(noteData + visibleOffset), 0];
		...
		if(Conductor.songPosition != lastTime || forceDataUpdate)
		{
			...
			for (note in curRenderedNotes)
			{
				...
				if(Conductor.songPosition > note.strumTime && lastTime <= note.strumTime)
				{
					...
					if(vortexPlaying)
					{
						var visibleStrumIndex:Int = getChartingVisualColumnIndex(Std.int(note.songData[1]));
						var strumNote:StrumNote = strumLineNotes.members[visibleStrumIndex];
						if(strumNote != null)
						{
	...
	{
		// Guard against invalid curSec or missing cached section data
		if(cachedSectionTimes == null || cachedSectionCrochets == null || cachedSectionRow == null) return;
		if(curSec < 0 || curSec >= cachedSectionTimes.length) return;
	...
	function refreshGridLayout()
	{
		...
		var columnCount:Int = getChartingVisibleTotalColumns() + (SHOW_EVENT_COLUMN ? 1 : 0);
		var stripes:Array<Int> = [];
		var columns:Int = 0;
		var chartPlayers:Int = getChartingPlayers();
		if(SHOW_EVENT_COLUMN) columns++;
		for (i in 0...chartPlayers)
		{
		...
		// First, cleanly calculate the true, centered X position for the main grid block
		gridBg.columns = columnCount;
		gridBg.loadGrid(gridColors[0], gridColors[1]);
		gridBg.screenCenter(X);
		gridBg.stripes = stripes.copy();
		gridBg.updateStripes();

		// Force the sections above and below to lock EXACTLY onto the main grid's centered X position
		if(prevGridBg != null)
		{
			prevGridBg.columns = columnCount;
			prevGridBg.loadGrid(gridColorsOther[0], gridColorsOther[1]);
			prevGridBg.x = gridBg.x; // Explicitly forces alignment
			prevGridBg.stripes = stripes.copy();
			prevGridBg.updateStripes();
		}
		
		if(nextGridBg != null)
		{
			nextGridBg.columns = columnCount;
			nextGridBg.loadGrid(gridColorsOther[0], gridColorsOther[1]);
			nextGridBg.x = gridBg.x; // Explicitly forces alignment
			nextGridBg.stripes = stripes.copy();
			nextGridBg.updateStripes();
		}

		if(timeLine != null)
		{
			timeLine.x = gridBg.x;
			timeLine.setGraphicSize(Std.int(Math.max(1, gridBg.width)), 4);
			timeLine.updateHitbox();
			timeLine.screenCenter(Y);
		}
		...
		if(vortexIndicator != null)
		{
			...
			vortexIndicator.y = FlxG.height / 2;
		}
		refreshStrumLineNotes();
		refreshHeaderIcons();

		if (SHOW_EVENT_COLUMN && gridBg != null)
		{
			// Automatically scans every basic engine component currently active on screen
			for (basic in this.members)
			{
				if (basic != null)
				{
					// 1. CATCH AND SNAP INDEPENDENT EVENT GROUPS
					if (Std.isOfType(basic, flixel.group.FlxGroup))
					{
						var group:flixel.group.FlxGroup = cast basic;
						group.forEach(function(subBasic:flixel.FlxBasic) {
							if (subBasic != null) {
								// Detects the colored event cube sprites by checking their size match
								if (Std.isOfType(subBasic, flixel.FlxSprite)) {
									var spr:flixel.FlxSprite = cast subBasic;
									if (spr.width == GRID_SIZE && spr.height == GRID_SIZE && spr != gridBg && spr != prevGridBg && spr != nextGridBg) {
										spr.x = gridBg.x + (GRID_SIZE * 0.5) - (spr.width / 2);
									}
								}
								// Detects the text labels resting right next to the event cubes
								else if (Std.isOfType(subBasic, flixel.text.FlxText)) {
									var txt:flixel.text.FlxText = cast subBasic;
									txt.x = gridBg.x + GRID_SIZE + 5;
								}
							}
						}, true);
					}
					// 2. CATCH LOOSE EVENT TEXT LABELS FLOATING ON THE CORE STATE
					else if (Std.isOfType(basic, flixel.text.FlxText))
					{
						var looseTxt:flixel.text.FlxText = cast basic;
						
						if (looseTxt.y > 100 && looseTxt.width > 50 && looseTxt.height < 30 && !looseTxt.text.contains('Press F1 for Help')) {
							looseTxt.x = gridBg.x + GRID_SIZE + 5;
						}
					}
	...
	function refreshHeaderIcons()
	{
		...
		{
			mustHitIndicator.y = iconY - 20;
			if(icons.length > 1)
			{
				var iconP1:HealthIcon = icons[0];
				var iconP2:HealthIcon = icons[1];
				var mustHitSection:Bool = (PlayState.SONG != null && PlayState.SONG.notes[curSec] != null && PlayState.SONG.notes[curSec].mustHitSection == true);
				mustHitIndicator.x = mustHitSection ? (iconP1.x + iconP1.width/2) : (iconP2.x + iconP2.width/2);
			}
		}
	}

	function refreshStrumLineNotes()
	{
	...
	function updateNotesForCurrentKeyCount(oldPlayerCount:Int, oldOpponentCount:Int, newPlayerCount:Int, newOpponentCount:Int)
	{
		if(notes == null) return;

		var currentSectionNotes:Array<MetaNote> = [];
		for (note in notes)
		{
			if(note == null) continue;
			var mappedData:Int = remapChartNoteDataForSides(Std.int(note.songData[1]), oldPlayerCount, oldOpponentCount, newPlayerCount, newOpponentCount);
			note.changeNoteData(mappedData);
			note.mustPress = getChartingMustPress(mappedData);
			note.reloadNote(note.texture);
			if (note.strumTime >= getMinNoteTime(curSec) && note.strumTime < getMaxNoteTime(curSec))
				currentSectionNotes.push(note);
		}

		for (note in currentSectionNotes)
		{
			positionNoteXByData(note);
		...
		for (event in events)
		{
			...
			if (event.strumTime >= getMinNoteTime(curSec) && event.strumTime < getMaxNoteTime(curSec))
				positionNoteYOnTime(cast event, curSec);
		}
	}
	...
	function loadChart(song:SwagSong)
	{
		...
		{
			var opponentCount:Int = Std.int(Song.getKeyCountForOpponent(PlayState.SONG));
			var playerCount:Int = Std.int(Song.getKeyCountForPlayer(PlayState.SONG));
			PlayState.SONG.opponentKeys = opponentCount;
			PlayState.SONG.playerKeys = playerCount;
			PlayState.SONG.keys = '$opponentCount;$playerCount';
		}
		StageData.loadDirectory(PlayState.SONG);
		Conductor.bpm = PlayState.SONG.bpm;
		if (cachedSectionTimes == null || cachedSectionBPMs == null || cachedSectionRow == null || cachedSectionCrochets == null || cachedSectionTimes.length < 1 || cachedSectionBPMs.length < 1)
		{
			_cacheSections();
		}
		refreshSongKeyCheckboxes();
		if (noOpponentCheckBox != null) noOpponentCheckBox.checked = (PlayState.SONG != null && PlayState.SONG.noOpponent == true);
		if (PlayState.SONG != null && gridBg != null && prevGridBg != null && nextGridBg != null && cachedSectionTimes != null && cachedSectionBPMs != null && cachedSectionRow != null && cachedSectionCrochets != null && cachedSectionTimes.length > 0 && cachedSectionBPMs.length > 0)
		{
			refreshGridLayout();
			loadSection();
			updateWaveform();
	...
	function createNote(note:Dynamic, ?secNum:Null<Int> = null)
	{
		...
		var daNoteData:Int = getChartingNoteData(note[1]);
		var gottaHitNote:Bool = getChartingMustPress(note[1]);

		// --- 🟩 GENERIC DATA OWNERSHIP SIDE-LOCK ---
		var noOpponent:Bool = (PlayState.SONG != null && PlayState.SONG.noOpponent == true);
		if (noOpponent && getChartingLaneCount() > 4) {
			// For broader multi-lane charts, the visible player side uses the first
			// set of lanes and the opponent side is hidden.
			gottaHitNote = (note[1] < getChartingColumnsPerPlayer());
		}
	...
	{
		if (cachedSectionBPMs == null || cachedSectionCrochets == null || cachedSectionRow == null || cachedSectionTimes == null || cachedSectionTimes.length < 1 || cachedSectionBPMs.length < 1)
		{
			_cacheSections();
		}
		if (PlayState.SONG == null || PlayState.SONG.notes == null || PlayState.SONG.notes.length < 1 || cachedSectionTimes == null || cachedSectionTimes.length < 1 || cachedSectionBPMs == null || cachedSectionBPMs.length < 1 || cachedSectionRow == null || cachedSectionCrochets == null)
		{
			if (prevGridBg != null) prevGridBg.visible = false;
			if (nextGridBg != null) nextGridBg.visible = false;
			if (gridBg != null) gridBg.visible = false;
			return;
		}
		...
		if(curSec > 0)
		{
			...
			if(eventLockOverlay != null && prevGridBg != null)
				eventLockOverlay.y = prevGridBg.y;
		}
		else if(prevGridBg != null) prevGridBg.visible = false;
		...
		else if(nextGridBg != null) nextGridBg.visible = false;
		...
		if(eventLockOverlay != null)
		{
			if(prevGridBg == null || !prevGridBg.visible)
				eventLockOverlay.y = gridBg != null ? gridBg.y : eventLockOverlay.y;
		...
		if(sec != null)
		{
			if(mustHitCheckBox != null) mustHitCheckBox.checked = sec.mustHitSection;
			if(gfSectionCheckBox != null) gfSectionCheckBox.checked = sec.gfSection;
			if(altAnimSectionCheckBox != null) altAnimSectionCheckBox.checked = sec.altAnim;
			if(changeBpmCheckBox != null) changeBpmCheckBox.checked = sec.changeBPM;
			if(changeBpmStepper != null) changeBpmStepper.value = Conductor.bpm;
			if(beatsPerSecStepper != null) beatsPerSecStepper.value = sec.sectionBeats;

			if(strumTimeStepper != null) strumTimeStepper.step = Conductor.stepCrochet;
			if(susLengthStepper != null)
			{
		...
		if(prevGridBg != null && gridBg != null && nextGridBg != null)
		{
	...
	function softReloadNotes(onlyCurrent:Bool = false)
	{
		...
		for (num => note in notes)
		{
			...
			{
				note.mustPress = getChartingMustPress(Std.int(note.songData[1]));
				if(PlayState.SONG != null && PlayState.SONG.noOpponent == true && !note.mustPress)
					continue;
		...
		if(SHOW_EVENT_COLUMN)
		{
			for (num => event in events)
			{
				if(event != null && curSecFilter(event))
				{
					...
					if (gridBg != null) {
						event.x = gridBg.x + (GRID_SIZE * 0.5) - (event.width / 2);
						if (event.eventText != null) {
							event.eventText.x = gridBg.x + GRID_SIZE - 150;
							
							event.eventText.fieldWidth = 100;
							event.eventText.wordWrap = true;
			...
			if(showPreviousSection || showNextSection)
			{
				...
				if(SHOW_EVENT_COLUMN)
				{
					for(event in events.filter(otherSecFilter))
					{
						...
						// --- INJECT THE BACKGROUND SECTION EVENT SNAPPER HERE ---
						if (gridBg != null) {
							event.x = gridBg.x + (GRID_SIZE * 0.5) - (event.width / 2);
							if (event.eventText != null) {
								event.eventText.x = gridBg.x + GRID_SIZE + 5;
							}
						}
	...
	function getMaxNoteTime(sec:Int)
	{
		...
		if(sec < cachedSectionTimes.length - 1)
	...
	function positionNoteXByData(note:MetaNote, ?data:Null<Int> = null)
	{
		...
		if (PlayState.SONG != null && PlayState.SONG.noOpponent == true)
			noteX += (GRID_SIZE * 0.5) - (note.width / 2);
		else
			noteX += (GRID_SIZE - note.width) / 2;

		var visualIndex:Int = getChartingVisualColumnIndex(Std.int(data));
		noteX += GRID_SIZE * (visualIndex - visibleOffset);
		note.x = noteX;
	...
	function updateJsonData():Void
	{
		...
		// Always populate character data for the full set of possible header icons
	...
	function updateHeads(ignoreCheck:Bool = false):Void
	{
		...
		// Always refresh every created header icon, even when noOpponent collapses
		// the chart to a single visible player. That keeps iconP2 populated for later use.
		for (i in 0...icons.length)
		{
			var icon:HealthIcon = icons[i];
			if(icon == null) continue;
	...
	function addSectionTab()
	{
		...
		{
			var maxData:Int = getChartingTotalColumns();
			var shiftBy:Int = (getChartingPlayers() > 1) ? getChartingColumnsPerPlayer() : 1;
			for (note in curRenderedNotes)
			{
				if(note != null && !note.isEvent)
				{
					var data:Int = note.songData[1] + shiftBy;
		...
		{
			var side:Int = -1;
			var columnsPerPlayer:Int = getChartingColumnsPerPlayer();
			for (note in curRenderedNotes.members)
			{
				if(note == null || note.isEvent) continue;

				if(getChartingPlayers() > 1)
				{
					if(side > -1)
					{
						if(Math.floor(note.songData[1] / columnsPerPlayer) != side)
						{
							showOutput('You cannot press this button with notes on more than one side.');
							return;
						}
					}
					else side = Math.floor(note.songData[1] / columnsPerPlayer);
				}
			}
			...
			for (note in curRenderedNotes.members)
			{
				...
				for (i in 0...getChartingPlayers())
				{
					if(getChartingPlayers() > 1 && i == side) continue;

					var songDataCopy:Array<Dynamic> = note.songData.copy();
					songDataCopy[1] = note.noteData + i * columnsPerPlayer;
		...
		var mirrorNotesButton:PsychUIButton = new PsychUIButton(objX + 200, objY, 'Mirror Notes', function()
		{
			...
			for (note in curRenderedNotes)
			{
				...
				note.changeNoteData((Math.floor(data / columnsPerPlayer) * columnsPerPlayer) + columnsPerPlayer - note.noteData - 1);
				positionNoteXByData(note);
			}
			softReloadNotes(true);
		});
	...
	var opponentSixKeyCheckBox:PsychUICheckBox;
	var playerSixKeyCheckBox:PsychUICheckBox;
	var noOpponentCheckBox:PsychUICheckBox;
	...
	function addSongTab()
	{
		...
		opponentSixKeyCheckBox = new PsychUICheckBox(objX + 140, objY + 140, '6-Key - Opponent', 140, function()
		{
			var prevSection:Int = curSec;
			var oldPlayerCount:Int = (PlayState.SONG != null) ? Std.int(Song.getKeyCountForPlayer(PlayState.SONG)) : 4;
			var oldOpponentCount:Int = (PlayState.SONG != null) ? Std.int(Song.getKeyCountForOpponent(PlayState.SONG)) : 4;
			PlayState.SONG.opponentKeys = opponentSixKeyCheckBox.checked ? 6 : 4;
			PlayState.SONG.keys = '${PlayState.SONG.opponentKeys};${PlayState.SONG.playerKeys}';
			refreshSongKeyCheckboxes();
			updateJsonData();
			refreshGridLayout();
			updateNotesForCurrentKeyCount(oldPlayerCount, oldOpponentCount, Std.int(PlayState.SONG.playerKeys), Std.int(PlayState.SONG.opponentKeys));
			loadSection(prevSection);
			updateWaveform();
			updateHeads(true);
		});
		playerSixKeyCheckBox = new PsychUICheckBox(objX + 140, objY + 160, '6-Key - Player', 120, function()
		{
			var prevSection:Int = curSec;
			var oldPlayerCount:Int = (PlayState.SONG != null) ? Std.int(Song.getKeyCountForPlayer(PlayState.SONG)) : 4;
			var oldOpponentCount:Int = (PlayState.SONG != null) ? Std.int(Song.getKeyCountForOpponent(PlayState.SONG)) : 4;
			PlayState.SONG.playerKeys = playerSixKeyCheckBox.checked ? 6 : 4;
			PlayState.SONG.keys = '${PlayState.SONG.opponentKeys};${PlayState.SONG.playerKeys}';
			refreshSongKeyCheckboxes();
			updateJsonData();
			refreshGridLayout();
		...
		noOpponentCheckBox = new PsychUICheckBox(objX + 140, objY + 180, 'No Opponent', 110, function()
		{
			PlayState.SONG.noOpponent = noOpponentCheckBox.checked;
			refreshGridLayout();
			loadSection(curSec);
		});
		...
		tab_group.add(opponentSixKeyCheckBox);
		tab_group.add(playerSixKeyCheckBox);
		tab_group.add(noOpponentCheckBox);
	...
	override function closeSubState()
	{
		...
		upperBox.bg.visible = false;
		updateAudioVolume();
	// Refresh character json data and header icons when returning from a substate
	updateJsonData();
	updateHeads(true);
	refreshHeaderIcons();
	}
	...
	{
		// --- 🟩 THE INTERCEPT & HEAL DATA REFACTOR ---
		if (data != null)
		{
			var playerCount:Int = (PlayState.SONG != null) ? Song.getKeyCountForPlayer(PlayState.SONG) : 4;
			var opponentCount:Int = (PlayState.SONG != null) ? Song.getKeyCountForOpponent(PlayState.SONG) : 4;

			// Extract whatever notes list array is hidden inside this specific action data structure
			var targetNotes:Array<MetaNote> = null;
			if (Reflect.hasField(data, 'notes')) targetNotes = data.notes;
			else if (Reflect.hasField(data, 'movedNotes')) targetNotes = data.movedNotes;
			else if (Reflect.hasField(data, 'originalNotes')) targetNotes = data.originalNotes;

			// If notes exist inside this payload package, loop through and clean their flags!
			if (targetNotes != null && targetNotes.length > 0)
			{
				for (note in targetNotes)
				{
					if (note != null)
					{
						// Calculate if this note truly belongs to the player using raw chart lane index.
					var truePlayerStatus:Bool = (note.songData[1] < playerCount);
						// Forcefully rewrite the underlying structural flags back to their absolute truth!
						note.mustPress = truePlayerStatus;
						@:privateAccess if (Reflect.hasField(note, 'playerNote')) Reflect.setField(note, 'playerNote', truePlayerStatus);
						@:privateAccess if (Reflect.hasField(note, 'mustHitSection')) Reflect.setField(note, 'mustHitSection', truePlayerStatus);
					}
				}
			}
		}
		...
		if(dataNotes != null && dataNotes.length > 0)
		{
			for (note in dataNotes)
			{
				if(note != null)
				{
					...
					// Ensure original stable index structures are committed first
					note.songData[0] = note.strumTime;
					var rawLane:Int = Std.int(note.songData[1]);
					note.chartNoteData = rawLane;
					note.noteData = rawLane % getChartingColumnsPerPlayer();
					note.mustPress = (rawLane < getChartingColumnsPerPlayer());
					trace("UNDO/REDO SCANNER -> Lane: " + note.chartNoteData + " | JSON Lane: " + note.songData[1] + " | mustPress: " + note.mustPress);
				}
			}
			notes.sort(PlayState.sortByTime);
		}
	...
	function updateWaveform() {
		...
		waveformSprite.y = gridBg.y;
		var width:Int = Std.int(GRID_SIZE * getChartingTotalColumns());
		...
		// Draws
		var totalColumns:Int = getChartingTotalColumns();
		var gSize:Int = Std.int(GRID_SIZE * totalColumns);
