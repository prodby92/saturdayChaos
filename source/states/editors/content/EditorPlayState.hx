...
class EditorPlayState extends MusicBeatSubstate
{
	...
	function generateSong()
	{
		...
		for (note in _noteList)
		{
			...
			var swagNote:Note = new Note(note.strumTime, note.noteData, oldNote, false, false, this, note.mustPress);
			...
			swagNote.animSuffix = note.animSuffix;
			...
			if(roundSus > 0)
			{
				for (susNote in 0...roundSus)
				{
					...
					var sustainNote:Note = new Note(note.strumTime + (curStepCrochet * susNote), note.noteData, oldNote, true, false, this, note.mustPress);
					...
					{
						var columnsPerPlayer:Int = (PlayState.SONG != null) ? Std.int(Math.max(4, Song.getKeyCountForOpponent(PlayState.SONG))) : 4;
						if (PlayState.SONG != null)
							columnsPerPlayer = Std.int(Math.max(4, Song.getKeyCountForOpponent(PlayState.SONG)));
						var visualLaneIndex:Int = Std.int(Math.abs(sustainNote.noteData) % columnsPerPlayer);
			...
			{
				var columnsPerPlayer:Int = (PlayState.SONG != null) ? Std.int(Math.max(4, Song.getKeyCountForOpponent(PlayState.SONG))) : 4;
				var visualLaneIndex:Int = Std.int(Math.abs(swagNote.noteData) % columnsPerPlayer);
				var halfCount:Int = Std.int(Math.ceil(columnsPerPlayer / 2));
				var laneIndex:Int = Std.int(Math.abs(visualLaneIndex) % halfCount);
				var sideOffset:Float = (columnsPerPlayer * 112) + 24;
				if (visualLaneIndex < halfCount)
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		var keyCount:Int = (PlayState.SONG != null) ? (player < 1 ? Song.getKeyCountForOpponent(PlayState.SONG) : Song.getKeyCountForPlayer(PlayState.SONG)) : 4;
		for (i in 0...keyCount)
		{
			// FlxG.log.add(i);
