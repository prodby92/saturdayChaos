...
class RGBPalette {
	...
	public function copyValues(tempShader:RGBPalette)
	{
		if (tempShader != null)
		{
			// Use setters to safely copy color values (they guard shader uniforms)
			this.r = tempShader.r;
			this.g = tempShader.g;
			this.b = tempShader.b;
			this.mult = tempShader.mult;
		}
		else
		{
			this.mult = 0.0;
	...
	private function set_mult(value:Float) {
		...
		if (shader != null && shader.mult != null) {
...
class RGBShaderReference
{
	...
	{
		parent = (ref != null) ? ref : new RGBPalette();
		_owner = owner;
		_original = parent;
		// Only assign the shader to the owner if we have both an owner and a valid shader
		if (_owner != null && parent != null && parent.shader != null) {
