...
class VisualsSettingsSubState extends BaseOptionsMenu
{
	...
	var noteY:Float = 90;
	var previousEditorSixKeyPalette:Bool = Note.forceEditorSixKeyPalette;
	public function new()
	{
		...
		for (i in 0...Note.colArray.length)
		{
			...
			note.force6KeyVisualMapping = true;
			changeNoteSkin(note);
			notes.add(note);
	...
	function playNoteSplashes()
	{
		...
		for (splash in splashes)
		{
			...
			var laneIndex:Int = splash.ID;
			var colorArray:Array<String> = Note.getColorArrayForNote();
			var colorKey:String = colorArray[(laneIndex % colorArray.length)];
			splash.inEditor = true;
			splash.spawnSplashNote(0, 0, laneIndex, null, false);
			splash.noteData = laneIndex;
			if (splash.babyArrow != null)
			{
				splash.x = splash.babyArrow.x - Note.swagWidth * 0.95;
				splash.y = splash.babyArrow.y - Note.swagWidth - 10;
			}
			if (splash.config != null)
			{
				var selectedAnim:String = null;
				for (anim in splash.config.animations)
				{
					if (anim.name != null && anim.name.toLowerCase() == colorKey.toLowerCase())
					{
				...
				{
					splash.noteData = laneIndex;
					splash.animation.play(selectedAnim, true);
					splash.applyAnimationOffsets(selectedAnim);
				}
			}

			var anim:String = splash.animation.curAnim != null ? splash.animation.curAnim.name : null;
			if (anim == null)
				anim = splash.playDefaultAnim();
	...
	override function destroy()
	{
		...
		Note.forceEditorSixKeyPalette = previousEditorSixKeyPalette;
